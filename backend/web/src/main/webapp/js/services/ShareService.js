  ...
  factory('ShareService', function (UtilsService) {
    var share = {
      vkontakte: function (item) {
        ...
        url += 'url=' + encodeURIComponent(UtilsService.generateUrl(item));
        ...
        if (UtilsService.isNotEmpty(item.thumbnailId)) {
          url += '&image=' + encodeURIComponent(UtilsService.thumbUrl(item));
        }
        url += '&noparse=true';
        share.popup(url);
      },
      ...
      facebook: function (item) {
        ...
        url += '&p[url]=' + encodeURIComponent(UtilsService.generateUrl(item));
        if (UtilsService.isNotEmpty(item.thumbnailId)) {
          url += '&p[images][0]=' + encodeURIComponent(UtilsService.thumbUrl(item));
        }
        share.popup(url);
      },
      twitter: function (item) {
        ...
        url += '&url=' + encodeURIComponent(UtilsService.generateUrl(item));
        url += '&counturl=' + encodeURIComponent(UtilsService.generateUrl(item));
        share.popup(url);
      },
